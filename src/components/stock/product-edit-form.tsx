...
export function ProductEditForm({ productId, defaultValues, categories, units }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const { updateProduct } = await import("@/lib/actions/stock")
      await updateProduct(productId, v)
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-zinc-200 p-5 space-y-4">
      <h3 className="font-semibold text-zinc-800 text-sm">Editar producto</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-zinc-600 mb-1">Precio venta</label>
          <input type="number" min="0" step="0.01" value={v.salePrice}
            onChange={e => set("salePrice", Number(e.target.value))}
            className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
        </div>
        <div>
          <label className="block text-xs font-medium text-zinc-600 mb-1">Precio mayorista</label>
        ...
        <div>
          ...
          <input type="number" min="0" step="1" value={v.stockMin}
