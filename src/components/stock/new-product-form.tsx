...
export function NewProductForm({ categories, units }: Props) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [v, setV] = useState({
    name: "", sku: "", categoryId: "", unitId: "",
    salePrice: 0, wholesalePrice: 0, stockCurrent: 0, stockMin: 0,
  })
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-zinc-200 p-5 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-zinc-600 mb-1">Precio de venta *</label>
          <input type="number" min="0" step="0.01" value={v.salePrice}
            onChange={e => set("salePrice", Number(e.target.value))} required
            className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900" />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="number" min="0" step="1" value={v.stockCurrent}
